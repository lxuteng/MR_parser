    ...
    def mro_ecid(self, object_mro, report_time, enbid):
        try:
            ...
            for value in object_mro:
                ...
                if temp_value[11] != '0' and temp_value[11] != 'NIL':
                    ...
                    # 统计邻区采样点
                    if temp_value[7] == temp_value[11]:
                        n_cell_id,n_cell_id_stance = self.min_distance_cell(
                            str(int(ecid1) // 256),
                            temp_value[11],
                            temp_value[12]
                        )
                        temp_ecid_ncell = '-'.join((ecid1, n_cell_id))
                        try:
                            self.temp_mro_data[report_time]['mro_ecid_mdt_sum'][temp_ecid_ncell][0] += 1
                        except:
                            try:
                                self.temp_mro_data[report_time]['mro_ecid_mdt_sum'][temp_ecid_ncell] = numpy.array([1, 0])
                            except:
                                try:
                                    self.temp_mro_data[report_time]['mro_ecid_mdt_sum'] = {}
                                    self.temp_mro_data[report_time]['mro_ecid_mdt_sum'][temp_ecid_ncell] = numpy.array([1, 0])
                                except:
                                    self.temp_mro_data[report_time] = {}
                                    self.temp_mro_data[report_time]['mro_ecid_mdt_sum'] = {}
            ...
            if ecid_long_lat != '':
                try:
                    ...
                    try:
                        ...
                        try:
                            self.temp_mro_data[report_time]['mro_ecid_mdt'] = {}
                            self.temp_mro_data[report_time]['mro_ecid_mdt'][ecid_long_lat] = 1
                        except:
                            self.temp_mro_data[report_time] = {}
                            self.temp_mro_data[report_time]['mro_ecid_mdt'] = {}
            ...
            try:
                for table in all_list['mro']:
                    if table == 'mro_main':
                        ...
                        temp_sum_counter = {}
                        for temp_report_time_temp in all_list['mro'][table]:
                            for table_id_temp in all_list['mro'][table][temp_report_time_temp]:
                                temp_value_temp = all_list['mro'][table][temp_report_time_temp][table_id_temp][3]
                        ...
                        with open(os.path.join(self.config_main['target_path'][0],
                            ...
                            writer = csv.writer(csvfile)
                            writer.writerow(('DAY', 'TIME', 'S_ECID', 'S_ENBID', 'S_ENB_CELLID', 'S_EARFCN', 'S_PCI',
                                             'N_ENB_CELLID', 'N_EARFCN', 'N_PCI', '站距', '是否同站邻区', 'Scrsrp',
                                             'ScTadv(m)', 'Ncrsrp',
                                            '是否mod3', '小于6db重叠覆盖度%', '邻区权重%','小区经度','小区纬度','小区方向角',
                                             'ue采样中心经度','ue采样中心纬度',
                                             ' N_Samplint', '<-10db', '-10db', '-9db', '-8db', '-7db',
                            ...
                            for temp_report_time in all_list['mro'][table]:
                                for table_id in all_list['mro'][table][temp_report_time]:
                                    ...
                                    if temp_value[3] != 0:
                                        ...
                                        temp_value_1 = [
                                            round(temp_value[0] / temp_value[3]-140, 2),
                                            int(temp_value[1] / temp_value[3]*78),
                                            round(temp_value[2] / temp_value[3]-140, 2)
                                        ]
                                    else:
                                    ...
                                    temp_mod_3 = '0'
                                    try:
                                        base_data_lon = float(self.mro_ecid_lon_lat_azi[temp_id[0]][0])
                                    except:
                                        base_data_lon = 0

                                    try:
                                        base_data_lat = float(self.mro_ecid_lon_lat_azi[temp_id[0]][1])
                                    except:
                                        base_data_lat = 0

                                    try:
                                        base_data_azi = int(self.mro_ecid_lon_lat_azi[temp_id[0]][2])
                                    except:
                                        base_data_azi = 0

                                    if temp_id[1] == temp_id[3] and int(temp_id[2]) % 3 == int(temp_id[4]) % 3:
                                        temp_mod_3 = '1'
                                    temp_value_4 = [
                                        temp_mod_3,
                                        round(sum(temp_value[10:21]) / temp_value[3] * 100, 2),
                                        round(temp_value[3] / temp_sum_counter[temp_id[0]] * 100, 2),
                                        base_data_lon,
                                        base_data_lat,
                                        base_data_azi,
                                        base_data_lon + math.sin(
                                            math.radians(base_data_azi)
                                        ) * 0.0000085 * 1.2 * (int(temp_value[1] / temp_value[3]*70)),
                                        base_data_lat + math.cos(
                                            math.radians(base_data_azi)
                                        ) * 0.0000085 * 1.04 * (int(temp_value[1] / temp_value[3] * 70))
                                    ]
                                    temp_value = list(temp_value_1) + list(temp_value_4) + list(temp_value[3:])
                        ...
                        if 'mro_ecid_mdt' in all_list['mro']:
                            with open(os.path.join(self.config_main['target_path'][0],
                                                   '{0}{1}_{2}.csv'.format('mro_ecid_mdt',
                                                                           temp_day_head,
                                                                           time_type)), 'w', newline='') as csvfile:
                                writer = csv.writer(csvfile)
                                writer.writerow((
                                    'DAY', 'TIME', 'ECID', 'ENBID', 'ENB_CELLID', 'CELL_NAME', 'lon',
                                    'lat', 'Sc_Earfcn', 'Sc_Pci', 'SC_RSRP', 'LteScRSRQ', 'LteScTadv',
                                    '总mod3比例%', '最大邻区mod3比例%',
                                    'A_邻区ID', 'A_邻区名', 'A_pci', 'A_邻区RSRP', 'A_邻区是否mod3', 'A_邻区重叠覆盖比例%',
                                    'A_邻区采样点数', 'A_邻区权重%',
                                    'B_邻区ID', 'B_邻区名', 'B_pci', 'B_邻区RSRP', 'B_邻区是否mod3', 'B_邻区重叠覆盖比例%',
                                    'B_邻区采样点数', 'B_邻区权重%',
                                    'C_邻区ID', 'C_邻区名', 'C_pci', 'C_邻区RSRP', 'C_邻区是否mod3', 'C_邻区重叠覆盖比例%',
                                    'C_邻区采样点数', 'C_邻区权重%',
                                    'D_邻区ID', 'D_邻区名', 'D_pci', 'D_邻区RSRP', 'D_邻区是否mod3', 'D_邻区重叠覆盖比例%',
                                    'D_邻区采样点数', 'D_邻区权重%',
                                    'E_邻区ID', 'E_邻区名', 'E_pci', 'E_邻区RSRP', 'E_邻区是否mod3', 'E_邻区重叠覆盖比例%',
                                    'E_邻区采样点数', 'E_邻区权重%',
                                    'F_邻区ID', 'F_邻区名', 'F_pci', 'F_邻区RSRP', 'F_邻区是否mod3', 'F_邻区重叠覆盖比例%',
                                    'F_邻区采样点数', 'F_邻区权重%',
                                    'G_邻区ID', 'G_邻区名', 'G_pci', 'G_邻区RSRP', 'G_邻区是否mod3', 'G_邻区重叠覆盖比例%',
                                    'G_邻区采样点数', 'G_邻区权重%',
                                    'H_邻区ID', 'H_邻区名', 'H_pci', 'H_邻区RSRP', 'H_邻区是否mod3', 'H_邻区重叠覆盖比例%',
                                    'H_邻区采样点数', 'H_邻区权重%',
                                ))
                                for temp_report_time in all_list['mro']['mro_ecid_mdt']:
                                    for ecid_id in all_list['mro']['mro_ecid_mdt'][temp_report_time]:
                                        temp_value = ecid_id.split('_')
                                        # temp_value_num = [all_list['mro']['mro_ecid_mdt'][temp_report_time][ecid_id]]
                                        ...
                                        try:
                                            ...
                                            temp_enb_cellid_cn_name = 'None'

                                        temp_writer_list = [temp_day, temp_report_time, temp_value[0],
                                                            temp_enbid,
                                                            temp_enb_cellid,
                                                            temp_enb_cellid_cn_name] + temp_value[1:8]

                                        temp_writer_n_list = []
                                        all_overlap_range = 0
                                        all_overlap_range_max_list = []

                                        for i in range(int(len(temp_value[8:])//3)):
                                            if i == 0:
                                                temp_num_1 = 0
                                                temp_num_2 = 3
                                            else:
                                                temp_num_1 = i*3
                                                temp_num_2 = (i+1)*3
                                            temp_n_cell = temp_value[8:][temp_num_1:temp_num_2]

                                            n_cell_id, n_temp_dis = self.min_distance_cell(
                                                str(int(temp_value[0])//256),
                                                str(temp_value[3]),
                                                str(temp_n_cell[2]))
                                            temp_writer_n_list.append(n_cell_id)
                                            try:
                                                n_cell_name = self.mro_enbcellid_cn_name[n_cell_id]
                                            except:
                                                n_cell_name = 'None'
                                            temp_writer_n_list.append(n_cell_name)
                                            temp_writer_n_list.append(temp_n_cell[2])
                                            temp_writer_n_list.append(temp_n_cell[0])
                                            temp_mod3_if = 0
                                            if int(temp_value[5]) >= int(self.config_mro['cmcc_overlap_scell_rsrp'][0]):
                                                if abs(int(temp_n_cell[0])-int(temp_value[5])) < int(self.config_mro[
                                                                                                         'cmcc_overlap_db'][0]):
                                                    if int(temp_value[4]) % 3 == int(temp_n_cell[2]) % 3:
                                                        temp_writer_n_list.append('1')
                                                        temp_mod3_if = 1
                                                    else:
                                                        temp_writer_n_list.append('0')
                                                else:
                                                    temp_writer_n_list.append('0')
                                            else:
                                                temp_writer_n_list.append('0')
                                            try:
                                                temp_s_n = '-'.join((temp_value[0],n_cell_id))
                                                temp_n_s_range_lsit = all_list['mro']['mro_ecid_mdt_sum'][temp_report_time][
                                                    temp_s_n]
                                                temp_n_s_range = round(temp_n_s_range_lsit[1]/temp_n_s_range_lsit[0]*100,2)
                                                temp_writer_n_list.append(str(temp_n_s_range))
                                                temp_writer_n_list.append(str(temp_n_s_range_lsit[0]))
                                                ecid_weight_factor = round(temp_n_s_range_lsit[0] / temp_sum_counter[temp_value[0]] * 100, 2)
                                                temp_writer_n_list.append(ecid_weight_factor)
                                                if ecid_weight_factor > int(self.config_mro[
                                                                                'ecid_overlap_weight_factor'][0]):
                                                    all_overlap_range += temp_n_s_range*temp_mod3_if
                                                    all_overlap_range_max_list.append(temp_n_s_range*temp_mod3_if)
                                            except:
                                                temp_writer_n_list.append('None')
                                        if all_overlap_range_max_list != []:
                                            all_overlap_range_max = str(max(all_overlap_range_max_list))
                                        else:
                                            all_overlap_range_max = '0'
                                        writer.writerow(temp_writer_list + [str(all_overlap_range),
                                                                            all_overlap_range_max] + temp_writer_n_list)
